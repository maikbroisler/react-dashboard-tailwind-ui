...
export function FileItem({ state, file }: FileItemProps) {
  const { onDeleteFileSelected } = useFileInput()
    ...
    <div className={container()}>
      ...
      {state === 'error' ? (
        <div className="flex flex-1 flex-col items-start gap-1">
          ...
          <button
            type="button"
            className="hover: text-error-700 hover:text-error-900 dark:text-error-400 dark:hover:text-error-300 text-sm font-semibold"
          >
            Try again
          </button>
        </div>
